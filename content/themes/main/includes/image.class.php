...
Class Image {
	 ...
	 */
	public static function render($image, $template = "standard", $echo = true) {
	...
	private static function renderSrcset($baseImageUrl, $sizes) {
		...
		if(!file_exists(PROJECT_ABSPATH . parse_url($baseImageUrl)['path'])){
			return '';
		}
	...
	private static function convertUrlToPath($url) {
		return PROJECT_ABSPATH . parse_url($url)['path'];
	}
}
